...
describe('#Eth', () => {
  ...
  describe('#getWalletFromMnemonic()', () => {
    it('should create a wallet from a mnemonic', async () => {
      ...
      expect(Eth.equals((await Eth.getWalletFromMnemonic(mnemonic, undefined, 1)).address, alice.address)).to.be.true
      expect(Eth.equals((await Eth.getWalletFromMnemonic(mnemonic, undefined, 2)).address, joe.address)).to.be.true
    })
  })
  ...
  describe('#getWalletFromPrivateKey()', () => {
    it('should create a wallet from a private key', async () => {
      ...
      const walletFromPrivateKey = await Eth.getWalletFromPrivateKey(privateKey)
  ...
  describe('#getWalletFromEncryptedJson()', () => {
    it('should create a wallet from an encrypted JSON', async () => {
      ...
      const walletFromEncryptedJson = await Eth.getWalletFromEncryptedJson(json, password)
