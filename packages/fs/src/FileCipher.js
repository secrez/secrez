...
class FileCipher {
  ...
  constructor(secrez) {
    if (secrez.constructor.name === 'Secrez') {
      ...
      throw new Error('FileCipher requires a Secrez instance during construction')
    }
  }
  ...
  decryptFile(encryptedContent, options) {
    ...
    const [version, content, passwordOrShortPublicKey, ...keys] = encryptedContent
    if (version === '1') {
      if (keys && keys.length) {
        ...
        for (let item of keys) {
          ...
          if (myShort === short) {
            key = myShort === passwordOrShortPublicKey
        ...
        let key = Crypto.decrypt(passwordOrShortPublicKey, this.getUint8ArrayPassword(options.password))
