...
const Entry = require("./Entry0");
const Crypto = require("./Crypto0");
const { ANCESTOR_NOT_FOUND, ENTRY_EXISTS } = require("./Messages0");
const DataCache = require("./DataCache0");
...
class Node0 {
  ...
  constructor(entry) {
    ...
    let isRoot = Node0.isRoot(entry);

    if (!(Node0.isDir(entry) || Node0.isFile(entry))) {
    ...
    if (isRoot) {
      ...
      if (
        ...
        !entry.encryptedName ||
        typeof entry.encryptedName !== "string"
      ) {
        throw new Error("Missing parameters");
      }

      if (Node0.isNode(entry.parent)) {
        // a Node0 can be independent of a tree.
  ...
  static fromJSON(json, secrez, allFiles) {
    ...
    json = Node0.preFormat(json, secrez, files);
    let root = Node0.initNode(json);
    return root;
  }
  ...
  static preFormat(
    ...
    json.V.sort(Node0.sortEntry);
    if (json.c) {
      for (let c of json.c) {
        Node0.preFormat(c, secrez, files, trash);
      }
    }
    return json;
  }
  ...
  static initNode(json, parent) {
    try {
      let V0 = json.V[0];
      let type = V0
        ? V0.type || parseInt(V0.encryptedName[0])
        : config.types.ROOT;
      let node = new Node0(
        new Entry({
          type,
      ...
      if (Node0.isDir(node)) {
        for (let i = 0; i < json.c.length; i++) {
          let n = Node0.initNode(json.c[i], node);
          if (n) {
            node.add(n);
          }
        }
  ...
  async find(options, list = []) {
    ...
    let re = Node0.getFindRe(options);
    let p = this.getPath();
          ...
          if (re.test(name)) {
            ...
            if (re.test(content || "")) {
              if (options.getNodes) {
                ...
                list.push([Node0.hashVersion(ts), p, undefined, true]);
              }
            }
          }
        }
  ...
  getVersions() {
    if (Node0.isRoot(this)) {
      ...
      return Object.keys(this.versions).sort(Node0.sortEntry);
    }
  }
  ...
  static getRoot(node) {
    if (Node0.isRoot(node)) {
      return node;
    } else {
      return Node0.getRoot(node.parent);
    }
  }

  static isNode(obj) {
    return obj instanceof Node0;
  }
  ...
  findChildById(id, includeThis) {
    ...
    if (includeThis && id === this.id) {
      ...
      for (let c in this.children) {
        ...
        if (Node0.isDir(child)) {
          let found = child.findChildById(id);
          if (Node0.isNode(found)) {
            return found;
          }
        }
      }
  ...
  getChildFromPath(p, returnCloserAncestor, dontThrow) {
    ...
    try {
      ...
      if (returnCloserAncestor && ancestorNode) {
        return [ancestorNode, p.slice(index).join("/")];
      }
  ...
  exists(p) {
    try {
      Node0.getRoot(this).getChildFromPath(p);
      return false;
    } catch (e) {
      return true;
    }
  ...
  getPathToChild(child) {
    if (!child || child.constructor.name !== "Node0") {
    ...
    if (
      child.id === config.specialId.ROOT &&
      child.rnd !== Node0.getRoot(this).rnd
    ) {
  ...
  addVersion(entry) {
    ...
    this.lastTs = Object.keys(this.versions).sort(Node0.sortEntry)[0];
    return this.lastTs;
  }
  ...
  remove(version = []) {
    ...
    if (this.parent) {
      ...
      for (let v in this.versions) {
        if (deleteAll || version.includes(Node0.hashVersion(v))) {
          result.push({
            id: this.id,
            version: Node0.hashVersion(v),
