...
describe('Server', async function () {
  ...
  beforeEach(async function () {
    ...
    await startHub()
    await startHub2()
    config = new Config({root: courierRoot, hub: `http://${localDomain}:4433`, owner: secrez1.getPublicKey()})
    server = new Server(config)
    await server.start()
    config2 = new Config({root: courierRoot2, hub: `http://${localDomain}:3344`, owner: secrez2.getPublicKey()})
