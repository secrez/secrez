...
class Config {
  constructor(options = {}) {
    ...
    if (process.env.NODE_ENV === "test" && root === defRoot) {
      throw new Error(
        "You are not supposed to test @secrez/courier in the default folder. This can lead to mistakes and loss of data."
      );
    }
    ...
    options.dbPath = path.join(options.dataPath, "courier.sqlite3");
