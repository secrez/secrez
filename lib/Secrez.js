...
function getEncryptedMasterKey() {
  ...
  return self.db.get(keys.MASTERKEY)
      .then(key => {
        if (key) {
          self.encryptedMasterKey = key
          self.set(READY)
        }
...
class Secrez {
  ...
  constructor(datadir = path.join(process.env.HOME, '.secrez')) {
...
`, 'utf-8')
          ...
          .then(() => {
            ...
            return getEncryptedMasterKey().bind(this)
          })
