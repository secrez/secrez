...
var InquirerCommandPrompt = require('../../../inquirer-command-prompt')
...
for (let v of _.values(Secret.contentFields())) {
  allCommands[context.SECRET].push('set ' + v)
}
allCommands[context.SECRET] = allCommands[context.SECRET].concat([
  'save',
  'cancel',
  'delete',
  '?'
...
class Commands {
  ...
  start() {
    ...
    return secrez.init()
        .then(() => {
          if (secrez.isReady()) {
            console.log(G('Welcome back.\n'))//Please login into your local account'))
            return this.login()
                .then(() => this.menu())
          } else {
  ...
  currentCommands() {
    ...
    return allCommands[this.context]
  }
    ...
    inquirer.prompt([
      {
        ...
        name: 'command',
        message: '>',
        autoCompletion: this.currentCommands(),
        ifTabsAndNoSuggestions: '?',
  ...
  formatLs(list, maxSize = 40) {
    ...
    return InquirerCommandPrompt.formatList(elems, maxSize, true)
  }
  ...
  set(param) {
    ...
    if (!param) {
      ...
      console.log(InquirerCommandPrompt.formatList(_.values(fields)))
      return this.menu()
    }
