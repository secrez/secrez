...
class Item extends require('./Section') {
  ...
  constructor(secrez, type) {
    ...
    this.availableCommands = _.defaults({
      ...
      set: {
        help: ['set [field]', 'Sets (overwrites) a new field', 'Use tab to autocomplete and see the available fields', 'To delete a field: ' + chalk.cyan('set password -d')],
        subCommands: []
      }
  ...
  save() {
    ...
    return this.secrez.setSecret(this.currentSecret)
        .then(() => {
          this.print('green','The new secret has been saved.')
